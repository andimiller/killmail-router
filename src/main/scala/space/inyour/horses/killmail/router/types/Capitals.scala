...
object Capitals {
  case class Capital(typeName: String, typeID: Int) derives Codec.AsObject
  ...
  def capitalShipEnricher(nodes: Vector[Capital]): Enricher = {
    val capitals: Set[Int] = nodes.map(_.typeID).toSet
    ...
    (j: Json) => {
      lazy val folder: Json.Folder[Json] = new Json.Folder[Json] {
        ...
        def onObject(value: JsonObject): Json  = {
          ...
          if (hasShipTypeId)
            value.add("is_capital", Json.fromBoolean(isCapital)).mapValues(_.foldWith(folder)).toJson
          else
